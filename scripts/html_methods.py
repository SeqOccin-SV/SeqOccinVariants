...
import get_tools_version as gtv
...
def tools_to_html(env_path, data_type) :
    tools = gtv.get_tools(env_path, data_type)
    text = ""
    for t in tools :
        text += "<p>" + t + "</p>\n"
    body = "<h2>Tools & versions</h2>\n"
    body += text
    body += "<br><br>"
    return body
...
def write_full_html(bam_stats_files, pbsv_stats_files, variantsizes_files, snp_stats_files, number, index, workdir, html = "stats/pipeline_output.html", datatype = "CLR") :
    ...
    Args:
        ...
        workdir (str) : Path to the directory containing the envs.
...
if __name__ == "__main__" :
    ...
    parser.add_argument("-w", "--workdir", help = "path to the running Snakefile's directory.", required = True)
    parser.add_argument("-d", "--datatype", help = "Type of data : CLR, hifi (or CCS), ONT.", required = True)
    ...
    write_full_html(args.bam, args.pbsv, args.variants, args.snp, args.number, args.index, args.workdir, args.html, args.datatype)
